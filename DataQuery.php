...
//quantity)
function getPriceHistoryByISBN($isbn) {
    ...
    //Initialize datasets
    $strAmtDS = "<dataset seriesname='aprice'>";
    $strQtyDS = "<dataset seriesName='tnlprice' parentYAxis='S'>";

    $strSQL = "select ISBN as ISBN,APRICE as APRICE,TNLPRICE as TNLPRICE,TULPRICE as TULPRICE,TIME as TIME from pricebackup where isbn in (select distinct isbn from textbook_good) ";// and flag='T' and isbn='".$isbn."'
    $result = mysql_query($strSQL) or die(mysql_error());
	
    if ($result) {
        while($ors = mysql_fetch_array($result)) {
            $strCat .= "<category label='" . $ors['ISBN'] . "'/>";

            //Generate the link
            $strLink = urlencode("javaScript:updateCharts(" . $ors['TIME'] . ");");
            $strAmtDS .= "<set value='" . $ors['TULPRICE'] . "' link='" . $strLink . "'/>";
            $strQtyDS .= "<set value='" . $ors['TULPRICE'] . "'/>";
        }
    }
    mysql_close($link);
